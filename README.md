...
- 헤더의 ‘내 샘플’에서 포켓몬 배치(샘플)와 여섯 마리 조합(파티)을 만들고 고치고 지웁니다. 이름·초성·도감 번호로 찾습니다. 저장은 먼저 이 브라우저에 남고, ‘동기화 켜기’로 받은 코드를 다른 기기의 ‘코드로 연결’에 넣으면 같은 샘플을 봅니다

자료 날짜를 누르면 API 생성 시각을 볼 수 있습니다. 상세 화면 아래의 ‘자료 시각 자세히’에서도 확인할 수 있습니다. API 생성 시각은 인게임 갱신 시각이 아닙니다.

즐겨찾기와 선택한 시즌/형식은 해당 브라우저에 저장됩니다. 통계는 인덱스 포함 최대 5건을 7일 이내로 임시 보관하며, 앱을 열거나 자료를 조회할 때 오래된 항목을 정리합니다. 브라우저 저장 공간 삭제 시 함께 지워집니다. 오프라인에서는 이전에 조회하여 남아 있는 자료만 볼 수 있으며 포켓몬 이미지는 표시되지 않을 수 있습니다. 샘플과 파티도 같은 브라우저 저장소를 쓰므로, 저장 공간을 지우면 함께 사라집니다.
