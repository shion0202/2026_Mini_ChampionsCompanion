...
const TAKEN_WITH = { def: '방어로 받음', spd: '특수방어로 받음' };
...
// 능력 포인트·성격·랭크 한 벌. key는 atk·spa·def·spd, hp는 성격·랭크가 없다.
...
const head = (title, reference, index, pokemon, speciesLabel) => {
  ...
  return (
    ...
    `<div><h3>${title}</h3>` +
    `<small>${species ? `${esc(speciesLabel(pokemon))} · ${species.types.map(t => TYPE_LABELS[t] ?? t).join(' · ')}` : '포켓몬을 고르세요'}</small></div></div>`
  );
};
...
// 특성은 그 포켓몬이 가질 수 있는 것을 모두 보인다. 계산에 영향이 없는 것도 고를 수 있다.
...
function attackerPanel(state, context) {
  ...
  return (
    ...
    `<button type="button" class="builds-pick" data-dmg-pick="move">` +
    `${move ? `${esc(move.label)} ${typeBadges([move.type])} <small>${CATEGORY_NAMES[move.category]} · 위력 ${move.power || '—'}</small>` : '기술 선택'}</button>` +
...
function defenderPanel(state, context) {
  ...
  return (
    ...
    `<strong data-dmg-out="hpPercent">${side.hpPercent}%</strong></div>` +
    `</fieldset>` +
...
export function damageResult(summary, context) {
  ...
  if (!attacker.pokemon || !defender.pokemon || !attacker.move)
    return '<div class="calc-verdict"><p class="calc-verdict-text">공격 측 포켓몬과 기술, 방어 측 포켓몬을 고르면 데미지를 보여줍니다.</p></div>';
  ...
  const head =
    ...
    `<div class="dmg-move"><strong>${esc(move.label)}</strong>${typeBadges([move.type])}` +
    `<small>기술 위력</small><b>${summary?.basePower ?? (attacker.power || move.power || '—')}</b>` +
    `<small>타입 상성</small><b>${summary?.effectiveness ?? '—'}×</b>` +
  ...
  const power =
    ...
    `<small class="dmg-sub">${STAT_NAMES_KO[context.keys.attack]} 실수치 ${summary.attackStat} × 위력 ${summary.basePower} × 자속 ${summary.stab}${hits > 1 ? ` · 타격당, ${hits}회` : ''}</small>` +
  ...
  const detail =
    ...
    `<div class="dmg-box"><small>난수 데미지${hits > 1 ? ' (1타당)' : ''}</small><div class="dmg-chips">${rolls}</div></div>` +
    `</div>`;
...
export function damageCalcView(state, summary, context) {
  ...
  return (
    ...
    `</div>` +
    `<div data-dmg-result>${result}</div>` +
    `<p class="speed-help">Showdown과 같은 순서로 계산합니다. 위력이 상황에 따라 바뀌는 기술(파워트립, 웨더볼 등)은 위력을 직접 넣어 주세요. 결정력은 공격 실수치 × 위력 × 자속, 내구력은 HP × 방어 ÷ 0.411입니다.</p>`
  );
}
