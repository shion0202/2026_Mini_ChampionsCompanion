...
기대: `정답 포착: 6 / 6`, 큐 크기 5000바이트 이하. 이 검사는 `.cache/`에 남으므로 커밋하지 않는다.

실행해 보면 두 가지가 걸린다. 둘 다 코드에 반영되어 있다.

- **파생 메가스톤을 점수에 넣으면 안 된다.** 폼에서 되짚은 스톤까지 도구로 세면 구축 경위에 환경 상위 메가로 이름만 오른 포켓몬(카이류, 갸라도스)이 실제 멤버를 밀어낸다. 점수를 먼저 매기고 스톤은 그 뒤에 채운다.
- **상한 12로는 멤버가 떨어진다.** 측정한 기사에서 실제 멤버인 `mimikyu`는 5회 등장에 근처 도구가 없어 10점, 상대 포켓몬인 `hippowdon`은 8회에 도구가 붙어 22점이었다. 텍스트 신호만으로는 둘을 가를 수 없으므로 점수를 손대지 않고 상한을 16으로 올린다. 대신 후보당 발췌를 하나로 줄여 큐 크기는 오히려 줄었다.
