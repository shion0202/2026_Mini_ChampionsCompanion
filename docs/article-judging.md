...
수집기(`npm run articles`)가 쌓은 큐를 Claude Code 세션이 판정하는 절차다. 등록 기준은
...
한 번에 요청받은 건수만 처리한다. 순위가 높은 기사부터.

### 1. 이미지 확인

`imageFiles` 중 null이 아닌 파일을 Read로 본다. 인게임 팀 화면(여섯 마리와 도구 아이콘)이나
