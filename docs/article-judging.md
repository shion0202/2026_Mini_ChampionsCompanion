...
- 원문이 열리지 않으면(인증서 경고, 삭제) 제목 옆 **사본 보기**로 수집기가 받아 둔 사본을
  ...
  `.cache/`에만 있다.
- 순위는 제목에서 읽는다(`最終N位`, `N位`. `最高N位`와 `2307.557位`처럼 레이팅에 붙은 것은
  읽지 않는다). 원문에 최종 순위가 아예 없으면 **순위 없음**을 체크한다. 비워 두면 모름이라
  추가되지 않는다.
- 블로그 첫 페이지로 기록된 것을 옮길 때는 화면의 값(여섯 칸, 순위 등)으로 저장한다.
  여섯 칸을 고친 뒤 [옮기기]를 눌러도 된다.
- 도구를 모르면 칸을 비운다. 빈칸이 있으면 추가되지 않는다(모름을 없음으로 바꾸지 않는다,
  [articles.md](articles.md) 4번). 도구가 없음이 확인되면 `도구 없음`을 고른다.

- 이미 공개한 기록을 다시 보려면 그 기록의 `review.status`를 `pending`으로 바꾼다. 검토
  화면 맨 앞에 기록 값 그대로 나오고, 앱에서는 다시 확정할 때까지 빠진다. 같은 글의 AI 제안이
  있으면 값은 덮지 않고 `AI 메모`에 기록과 다른 칸(포켓몬·도구·작성자·순위)만 적는다.
  포케솔 기사면 `npm run propose`가 pending 기록에도 제안을 쓴다.
