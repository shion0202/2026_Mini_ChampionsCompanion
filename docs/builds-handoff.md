...
샘플 편집기의 채용·후보 기술 줄 왼쪽 손잡이(⠿)를 잡고 끈다. 같은 목록 안에서는
순서를 옮기고, 채용과 후보 사이에서는 놓은 기술과 맞바꾼다. 채용 빈 칸에 놓으면
그 칸을 채우고, 후보 칸의 빈 곳에 놓으면 후보 끝으로 간다. 규칙은
`src/builds.js`의 `dragMove` 하나에 있고, `src/app.js`는 포인터 이벤트로 놓을 자리만
찾는다. 폰에서 되어야 하므로 HTML 기본 드래그(마우스 전용)를 쓰지 않는다.
