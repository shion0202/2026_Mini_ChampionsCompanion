...
절차는 `docs/builds-deploy.md`에 있다. 주소창 없이 앱처럼 여는 것뿐이라 급하지
않다. 지금도 폰 브라우저에서 PC 없이 열리고 ‘홈 화면에 추가’로 아이콘을 둘 수
있다. 필요한 것은 서명 키의 **SHA-256 지문**이며, 지문은 비밀이 아니다. 키와
비밀번호는 대화에 붙여넣지 않는다.
