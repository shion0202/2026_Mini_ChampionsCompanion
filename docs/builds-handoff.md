...
설계는 `docs/builds-and-sync.md`의 `저장과 동기화`, `API` 절에 있다.
...
두므로 1초 안에 두 번 저장하면 두 번째가 거절된다. 연속 저장을 모아 보내고,
거절되면 다시 시도해야 한다.

사람이 해야 하는 것은 KV 네임스페이스를 만들고 Pages 프로젝트에 묶는 것 하나다.
코드가 준비된 뒤에 한다.
...
- **`sample.form`** — 모델에만 있고 쓰이지 않는다. 폼은 `pokemon` id가 이미
  구분한다(`charizardmegax`). 저장된 자료의 모양이 바뀌므로 3단계에서 저장 모양을
  정할 때 `normalizeDoc`과 함께 지운다.
